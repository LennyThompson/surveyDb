...
public class SurveyReferencesHttpHandler extends HandlerBase implements HttpHandler
{
    ...
    private void getSurveyReferencesRequest(HttpExchange httpExchange)
    {
        try
        {
            ...
            {
                ISurveyReference SurveyReference = SurveyReferenceAdapter.get(ConnectionManager.getInstance().getConnection(), Integer.parseInt(getRequestMap().get("ID").getValue()));
