...
public class SurveyAdjustmentsHttpHandler extends HandlerBase implements HttpHandler
{
    ...
    private void getSurveyAdjustmentsRequest(HttpExchange httpExchange)
    {
        try
        {
            ...
            {
                ISurveyAdjustment SurveyAdjustment = SurveyAdjustmentAdapter.get(ConnectionManager.getInstance().getConnection(), Integer.parseInt(getRequestMap().get("ID").getValue()));
